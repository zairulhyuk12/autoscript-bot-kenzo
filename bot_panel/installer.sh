...
surat=$(curl -sS https://raw.githubusercontent.com/zairulhyuk12/autoscript-bot-kenzo/ip-bot/main/bot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -e "KenZo Zairul Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @KenzoZairul] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@KenzoZairul"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: KenZo Zairul-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="KenZo Zairul-STORE"
        echo ""
